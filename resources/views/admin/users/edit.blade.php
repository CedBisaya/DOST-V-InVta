...
<div class="max-w-5xl mx-auto pb-10 px-4">
    {{-- Return Button --}}
    <div class="mb-6">
        <a href="#" class="flex items-center text-gray-400 hover:text-cyan-500 transition-colors font-bold text-xs uppercase tracking-widest">
    ...
    <div class="bg-white rounded-[1rem] shadow-sm border border-gray-100 p-6 md:p-8 mb-6">
        ...
        <form method="post" action="#" class="space-y-6">
            ...
            {{-- Responsive Grid: stacking on mobile (grid-cols-1), 4 columns on desktop --}}
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-6 mt-3">
                        <label class="flex items-center cursor-pointer group">
                            <input type="radio" name="role" value="external" class="w-4 h-4 text-cyan-500 border-gray-300 focus:ring-cyan-400">
                            <span class="ml-2 text-sm text-gray-600 group-hover:text-cyan-500 transition-colors">External</span>
                        </label>
                        <label class="flex items-center cursor-pointer group">
                            <input type="radio" name="role" value="internal" class="w-4 h-4 text-cyan-500 border-gray-300 focus:ring-cyan-400">
    ...
    <div class="bg-white rounded-[1rem] shadow-sm border border-gray-100 p-6 md:p-8">
        ...
        <form action="#" method="POST">
            @csrf
            @method('PATCH')
            {{-- Fixed: Changed to flex-col for mobile, flex-row for desktop --}}
...
{{-- halion ini, nata rayaon ini sa sarong file --}}
@if(session('success'))
<div id="success-modal" class="fixed inset-0 flex items-center justify-center z-[100] transition-opacity duration-300">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-10 max-w-sm w-full mx-4 relative transform transition-all scale-100 flex flex-col items-center">
        <button onclick="closeModal()" class="absolute top-6 right-6 text-gray-400 hover:text-gray-600">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
        <div class="w-20 h-20 md:w-24 md:h-24 rounded-full border-[6px] border-green-500 flex items-center justify-center mb-6">
            <svg class="w-10 h-10 md:w-14 md:h-14 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M5 13l4 4L19 7" /></svg>
        </div>
        <h3 class="text-[20px] md:text-[24px] font-black text-green-500 tracking-tight text-center leading-tight">Save Changes Successfully!</h3>
    </div>
</div>
