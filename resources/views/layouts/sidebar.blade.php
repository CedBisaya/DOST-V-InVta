...
<aside 
    class="fixed inset-y-0 left-0 z-50 bg-white border-r border-gray-100 flex flex-col transition-all duration-300 ease-in-out transform lg:static lg:translate-x-0 shadow-xl lg:shadow-none"
    :class="sidebarOpen ? 'w-60 translate-x-0' : '-translate-x-full lg:translate-x-0 lg:w-16'"
>
    <div class="flex items-center justify-between h-16 px-4 shrink-0 overflow-hidden border-b border-gray-50">
        <div class="flex items-center min-w-0">
            <div class="shrink-0 transition-all duration-300" :class="sidebarOpen ? 'opacity-100 w-auto' : 'lg:opacity-0 lg:w-0'">
    ...
    <nav class="flex-1 mt-3 space-y-0.5 overflow-y-auto custom-scrollbar overflow-x-hidden">
        @php
            $navItems = [
                ...
                ['name' => 'Event Manager', 'route' => 'events.index', 'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
        ...
        @foreach($navItems as $item)
            ...
            class="group relative flex items-center px-3 py-2.5 text-gray-500 hover:bg-dost-hover hover:text-dost-cyan transition-all duration-200 overflow-hidden">
                ...
                <div class="absolute left-0 w-1 h-5 bg-dost-cyan opacity-0 group-hover:opacity-100 rounded-r-full transition-opacity"></div>
            </a>
        @endforeach
    </nav>
